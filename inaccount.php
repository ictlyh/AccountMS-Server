...
<html lang="zh-cn">
	...
	<body>
				...
				if (isset($_POST['action'])){
					...
					if ($_POST['action']=='remove'&&isset($_POST['del'])){
						foreach ($_POST['del'] as $item){
							//删除收入条目
							$db->query("DELETE FROM tb_inaccount WHERE userID='$username' AND _id='$item'");
						}
				?>
					<div class="alert alert-success fade in" style="text-align: center;">
						<a class="close" data-dismiss="alert" href="#">x</a>
						<strong>成功删除收入</strong>
					</div>
		<?php
					}
				}
		...
		<!-- 显示收入信息	-->
			...
			<p><h3>您的收入：</h3></p>
		<?php
			$record=$db->query("SELECT * FROM tb_inaccount WHERE userID='$username'");
			if ($record->num_rows==0){//无收入
		?>
			<div class="alert alert-error fade in" style="text-align: center;">
				<a class="close" data-dismiss="alert" href="#">x</a>
				<strong>您目前没有任何收入</strong>
			</div>
			<button class="btn btn-large btn-danger"><a href="#addinaccount" data-toggle="modal">新增收入</a></button>
		<?php
			}else{//显示收入列表
		?>
				<form method="post" action="./inaccount.php">
					...
					<button type="submit" class="btn btn-large btn-danger">删除收入</button>
					<button class="btn btn-large btn-danger"><a href="#addinaccount" data-toggle="modal">新增收入</a></button>
				</form>
	<?php
				}
